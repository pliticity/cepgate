(function () {
    ...
    documentApp.controller('DocumentController', ['$cookies', '$http', '$scope', 'Upload', '$compile', '$uibModal', function ($cookies, $http, $scope, Upload, $compile, $uibModal) {
        $http.defaults.headers.common['A-TOKEN'] = $cookies.get('A-TOKEN');
        ...
        $http({method: 'GET', url: '/documents/docNo'}).then(function successCallback(response) {
            console.log(response);
            $scope.documentInfo.masterDocumentNumber = angular.toJson(response.data,false);
            $scope.documentInfo.documentNumber = angular.toJson(response.data,false);
        });
        doc.recentDocuments = [];
        doc.searchDocuments = [];
        doc.myDocuments = [];
        ...
        $http.get('/signup/as6d').success(function (response) {
            console.log('app-document-authenticated2');
        ...
        $http.get('/signup/authenticated').success(function (response) {
            console.log('app-document-authenticated');
            if (response.result == "false") {
                window.location = "/signup/login";
            }
        });
        ...
        doc.getNewDocument = function () {
            var aDoc = {
                "caseId": "", "caseName": "", "documentName": "", "documentName2": "", "documentName3": "",
        ...
        doc.saveDocuments = function (theType) {
            ...
            if (isGood) {
                ...
                $http({
                    method: 'POST',
                    url: theURL,
                    data: angular.toJson($scope.documentInfo, false)
                });

                /*	 Upload.upload({
                 url: '/documents/file/upload',
                 data:  $scope.file
                 }).then(function (resp) {
